                ...
                with st.container():
                    ...
                    with col2:
                        ...
                        acc_response = requests.get(
                            f"{API_URL}/users/{dep['userID']}/accounts")
                        ...
                        txn_response = requests.get(
                            f"{API_URL}/transactions/user/{dep['userID']}")
                        if txn_response.status_code == 200:
                            ...
                            if txns:
                                ...
                                for t in recent:
                                    ...
                                    if amt > 0:
                                        st.caption(
                                            f"• {t['description']}: :green[+${amt:,.0f}]")
                                    else:
                                        st.caption(
                                            f"• {t['description']}: :red[-${abs(amt):,.0f}]")
                            else:
...
try:
    ...
    if main_response.status_code == 200:
        ...
        with col2:
            st.metric("Household Spending",
                      f"${main_totals['expenditures']:,.0f}")
        with col3:
            st.metric("Household Savings",
                      f"${main_totals['netPosition']:,.0f}")
