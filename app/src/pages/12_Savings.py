    ...
    if response.status_code == 200:
        ...
        if st.session_state.get('show_saving_form', False):
            with st.form("saving_form"):
                ...
                target_amount = st.number_input(
                    "Target Amount ($)", min_value=1.0, step=100.0)
                current_amount = st.number_input(
                    "Current Amount ($)", min_value=0.0, step=10.0)
                monthly_contribution = st.number_input(
                    "Monthly Contribution ($)", min_value=0.0, step=10.0)
        ...
        if savings:
                ...
                if action == 'add':
                    with st.form("add_money_form"):
                        ...
                        amount = st.number_input(
                            "Amount to Add ($)", min_value=0.01, step=10.0, value=10.0)

                        col1, col2 = st.columns(2)
                        with col1:
                            add_submitted = st.form_submit_button(
                                "Add Money", type="primary")
                        with col2:
                            add_cancelled = st.form_submit_button("Cancel")

                        if add_submitted:
                            response = requests.put(
                                f"{API_URL}/savings/{saving_id}/add", json={"amount": float(amount)})
                            if response.status_code == 200:
                                st.success(
                                    f"Added ${amount:,.2f} to {goal_name}!")
                                st.session_state['modifying_saving'] = None
                                st.session_state['modify_action'] = None
                                st.rerun()
                            else:
                    ...
                    with st.form("remove_money_form"):
                        ...
                        amount = st.number_input(
                            "Amount to Remove ($)", min_value=0.01, max_value=max_val, step=10.0, value=min(10.0, max_val))

                        col1, col2 = st.columns(2)
                        with col1:
                            remove_submitted = st.form_submit_button(
                                "Remove Money", type="primary")
                        with col2:
                            remove_cancelled = st.form_submit_button("Cancel")

                        if remove_submitted:
                            response = requests.put(
                                f"{API_URL}/savings/{saving_id}/remove", json={"amount": float(amount)})
                            if response.status_code == 200:
                                st.success(
                                    f"Removed ${amount:,.2f} from {goal_name}!")
                                st.session_state['modifying_saving'] = None
                                st.session_state['modify_action'] = None
                                st.rerun()
                            else:
            ...
            for saving in savings:
                ...
                monthly = float(saving['monthlyContribution']
                                ) if saving['monthlyContribution'] else 0
                ...
                with col1:
                    ...
                    st.caption(
                        f"${current:,.0f} / ${target:,.0f} ({progress*100:.0f}%)")
                    if monthly > 0:
                        st.caption(f"Monthly contribution: ${monthly:,.0f}")
                    if saving.get('targetDeadline'):
                        st.caption(
                            f"Target date: {saving['targetDeadline'][:10]}")
