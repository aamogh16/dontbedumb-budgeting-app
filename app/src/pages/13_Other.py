...
for i, (col, name) in enumerate(zip(cols, tab_names)):
    with col:
        if i == current_tab:
            st.button(
                name, key=f"other_tab_{i}", type="primary", use_container_width=True, disabled=True)
        else:
...
if current_tab == 0:
    ...
    if st.session_state.get('show_debt_form', False):
        with st.form("debt_form"):
            ...
            amount = st.number_input(
                "Total Amount ($)", min_value=0.01, step=100.0)
            amount_paid = st.number_input(
                "Amount Already Paid ($)", min_value=0.0, step=100.0)
            interest_rate = st.number_input(
                "Interest Rate (%)", min_value=0.0, max_value=100.0, step=0.1)
            min_payment = st.number_input(
                "Minimum Payment ($)", min_value=0.0, step=10.0)
            ...
            if acc_response.status_code == 200:
                ...
                if accounts:
                    account_options = {
                        f"{a['accType']} - {a['institution']}": a['acctID'] for a in accounts}
    ...
    if st.session_state.get('paying_loan'):
        ...
        with st.form("pay_loan_form"):
            ...
            payment_amount = st.number_input("Payment Amount ($)", min_value=0.01, max_value=float(
                remaining), step=10.0, value=min(100.0, float(remaining)))
            ...
            if pay_submitted:
                ...
                response = requests.put(
                    f"{API_URL}/loans/{loan_id}/pay", json={"amount": float(payment_amount)})
                if response.status_code == 200:
                    ...
                    if acc_response.status_code == 200:
                        ...
                        if accounts:
                            ...
                            requests.post(
                                f"{API_URL}/transactions/", json=txn_data)

                    st.success(
                        f"Payment of ${payment_amount:,.2f} applied to {loan_name}!")
                    st.session_state['paying_loan'] = None
                    st.rerun()
                else:
    ...
    if response.status_code == 200:
        ...
        if loans:
            total_debt = sum(float(l['amount']) -
                             float(l['amountPaid']) for l in loans)
            ...
            for loan in loans:
                ...
                progress = float(
                    loan['amountPaid']) / float(loan['amount']) if float(loan['amount']) > 0 else 0
                ...
                with col1:
                    ...
                    st.caption(
                        f"Paid: ${float(loan['amountPaid']):,.0f} / ${float(loan['amount']):,.0f}")
                ...
                with col2:
                    ...
                    if loan.get('minPayment'):
                        st.caption(
                            f"Min Payment: ${float(loan['minPayment']):,.0f}")
                with col3:
                    if remaining > 0:
    ...
    if st.session_state.get('show_inv_form', False):
        with st.form("investment_form"):
            ...
            account_type = st.selectbox(
                "Account Type", ["Retirement", "Taxable", "Education", "Other"])
            balance = st.number_input(
                "Current Balance ($)", min_value=0.0, step=100.0)
            return_rate = st.number_input(
                "Return Rate (%)", min_value=-100.0, max_value=100.0, step=0.1)
            ...
            if submitted:
                if name:
                    ...
                    post_response = requests.post(
                        f"{API_URL}/investments/", json=data)
                    if post_response.status_code == 201:
        ...
        with st.form("subscription_form"):
            ...
            if submitted:
                if name and account_id:
                    ...
                    post_response = requests.post(
                        f"{API_URL}/subscriptions/", json=data)
                    if post_response.status_code == 201:
        ...
        if subscriptions:
            ...
            for sub in subscriptions:
                ...
                if st.session_state.get('paid_sub_id') == sub['subID']:
                    st.success(
                        f"✅ Paid ${float(sub['amount']):,.2f} for {sub['name']}!")
...
# Bills Tab
    ...
    if response.status_code == 200:
        ...
        if bills:
            unpaid_total = sum(float(b['amount'])
                               for b in bills if not b['isPaid'])
