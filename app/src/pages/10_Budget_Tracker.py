...
def edit_total_budget(budget):
    ...
    st.session_state['edit_total_limit'] = float(
        budget.get('limitAmount', 0)) if budget else 0.0
...
def edit_category_budget(cat_id, cat_name, existing):
    ...
    st.session_state['edit_category_limit'] = float(
        existing.get('limitAmount', 0)) if existing else 0.0
...
def convert_to_csv(transactions):
    ...
    columns_to_export = ['date', 'description', 'amount',
                         'categoryName', 'method', 'source', 'budgetName']
...
# Tab navigation
tab_names = ["Totals", "Budget Manager",
             "Income", "Expenditures", "All Transactions"]
...
for i, (col, name) in enumerate(zip(cols, tab_names)):
    with col:
        if i == current_tab:
            st.button(name, key=f"tab_{i}", type="primary",
    ...
    if response.status_code == 200:
        ...
        with col1:
            st.metric("Total Income",
                      f"${float(totals.get('income', 0)):,.0f}")
        with col2:
            st.metric("Total Expenses",
                      f"${float(totals.get('expenditures', 0)):,.0f}")
        with col3:
            st.metric("Net", f"${float(totals.get('netPosition', 0)):,.0f}")
        with col4:
            st.metric("Savings Rate",
                      f"{float(totals.get('savingsRate', 0)):,.0f}%")
    else:
        ...
        if budgets:
            for budget in budgets:
                ...
                if limit_amount and float(limit_amount) > 0:
                    ...
                    st.write(
                        f"**{type_icon}{budget.get('categoryName') or budget.get('name', 'Budget')}**")
    ...
    if response.status_code == 200:
        ...
        if categories:
            for cat in categories:
                ...
                st.write(
                    f"**{cat.get('category', 'Unknown')}**: ${spent:,.0f}")
        else:
...
# Budget Manager Tab
    ...
    total_budget = next(
        (b for b in budgets if b.get('budgetType') == 'total'), None)
    ...
    if st.session_state.get('edit_total_budget') is not None or (st.session_state.get('edit_total_budget') is None and total_budget is None and st.session_state.get('creating_total')):
        ...
        with col1:
            if total_budget:
                st.write(
                    f"**Total Budget**: ${float(total_budget.get('limitAmount', 0)):,.0f}")
            else:
    ...
    cat_budgets = {b.get('categoryID'): b for b in budgets if b.get(
        'budgetType') == 'category'}
    ...
    for cat_name, cat_id in EXPENSE_CATEGORIES.items():
        ...
        with col1:
            if existing and existing.get('limitAmount'):
                st.write(
                    f"**{cat_name}**: ${float(existing.get('limitAmount', 0)):,.0f}")
            else:
    ...
    if custom_budgets:
        for budget in custom_budgets:
            ...
            with col1:
                st.write(
                    f"**{budget.get('name', 'Budget')}**: ${float(budget.get('limitAmount', 0)):,.0f}")
            with col2:
...
# Income Tab
    ...
    with col_export:
        response = requests.get(
            f"{API_URL}/transactions/user/{user_id}/income")
        if response.status_code == 200:
    ...
    search_income = st.text_input(
        "🔍 Search income", placeholder="Search by description, source, amount...", key="search_income")
        ...
        with st.form("income_form"):
            ...
            if submitted:
                acc_response = requests.get(
                    f"{API_URL}/users/{user_id}/accounts")
                if acc_response.status_code == 200:
                    ...
                    if accounts:
                        ...
                        post_response = requests.post(
                            f"{API_URL}/transactions/", json=data)
                        if post_response.status_code == 201:
    ...
    if response.status_code == 200:
        ...
        if not search_income:
            ...
            if source_response.status_code == 200:
                ...
                for src in sources:
                    st.write(
                        f"**{src.get('source', 'Unknown')}**: ${float(src.get('total', 0)):,.0f}")

        st.write(
            "#### Recent Income" if not search_income else f"#### Search Results ({len(filtered_income)})")
        for txn in filtered_income[:15]:
            col1, col2, col3 = st.columns([3, 1, 1])
            with col1:
                st.write(f"**{txn.get('description', 'No description')}**")
                st.caption(
                    f"{txn.get('source') or 'Unknown'} • {format_date(txn.get('date'))}")
            with col2:
...
# Expenditures Tab
    ...
    with col_export:
        response = requests.get(
            f"{API_URL}/transactions/user/{user_id}/expenses")
        if response.status_code == 200:
    ...
    search_expense = st.text_input(
        "🔍 Search expenses", placeholder="Search by description, category, amount...", key="search_expense")
    ...
    if st.session_state.get('show_expense_form', False):
        with st.form("expense_form"):
            ...
            method = st.selectbox("Payment Method", [
                                  "Debit Card", "Credit Card", "Cash", "Auto-pay", "Transfer"])
            date = st.date_input("Date")

            budget_id = None
            budget_response = requests.get(
                f"{API_URL}/budgets/user/{user_id}/list")
            if budget_response.status_code == 200:
                budgets_list = budget_response.json()
                if budgets_list:
                    budget_options = {"None": None}
                    budget_options.update(
                        {b.get('name', 'Budget'): b.get('budgetID') for b in budgets_list})
                    selected_budget = st.selectbox(
                        "Add to Budget (optional)", list(budget_options.keys()))
    ...
    if response.status_code == 200:
        ...
        total_expenses = sum(abs(float(t.get('amount', 0)))
                             for t in filtered_expenses)
        ...
        if not search_expense:
            ...
            if cat_response.status_code == 200:
                ...
                for cat in categories[:5]:
                    st.write(
                        f"**{cat.get('category', 'Unknown')}**: ${float(cat.get('total', 0)):,.0f}")

        st.write(
            "#### Recent Expenses" if not search_expense else f"#### Search Results ({len(filtered_expenses)})")
        for txn in filtered_expenses[:15]:
            col1, col2, col3 = st.columns([3, 1, 1])
            with col1:
                st.write(f"**{txn.get('description', 'No description')}**")
                budget_label = f" • 🎯 {txn.get('budgetName')}" if txn.get(
                    'budgetName') else ""
                st.caption(
                    f"{txn.get('categoryName') or 'Uncategorized'} • {format_date(txn.get('date'))}{budget_label}")
            with col2:
...
# All Transactions Tab
    ...
    search_all = st.text_input(
        "🔍 Search transactions", placeholder="Search by description, category, source, amount...", key="search_all")
    ...
    if response.status_code == 200:
        ...
        income_total = sum(float(t.get('amount', 0))
                           for t in filtered_all if float(t.get('amount', 0)) > 0)
        expense_total = sum(abs(float(t.get('amount', 0)))
                            for t in filtered_all if float(t.get('amount', 0)) < 0)
        ...
        st.write(
            "#### Transactions" if not search_all else f"#### Search Results ({len(filtered_all)})")
        for txn in filtered_all[:25]:
            col1, col2, col3 = st.columns([3, 1, 1])
            with col1:
                st.write(f"**{txn.get('description', 'No description')}**")
                category_or_source = txn.get('categoryName') or txn.get(
                    'source') or 'Uncategorized'
                budget_label = f" • 🎯 {txn.get('budgetName')}" if txn.get(
                    'budgetName') else ""
                st.caption(
                    f"{category_or_source} • {format_date(txn.get('date'))}{budget_label}")
            with col2:
